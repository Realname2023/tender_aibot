...
agent = create_agent(
    ...
    system_prompt="""Ты ассистент по анализу госзакупок Казахстана для поставщиков.
                ...
                КРИТЕРИИ ОТБОРА — закупка НЕ интересна если:
                - есть квалификационные требования по материальным ресурсам
                - есть квалификационные требования по трудовым ресурсам
                - закупка не соответствует запросу пользователя
                
                ФОРМАТ ОТВЕТА — если закупка НЕ интересна всегда ответ начинай с: "False".
